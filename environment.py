...
@dataclass
class Position:
    """A 2D integer coordinate within the grid."""
    x: int
    y: int
    ...
    def __lt__(self, other):
        """Ordering to allow use in priority queues and sorted collections."""
        if not isinstance(other, Position):
...
class DynamicObstacle:
    ...
    def get_position_at_time(self, time: int) -> Position:
        """Return the obstacle position at the given time (cyclic schedule)."""
...
class GridEnvironment:
    """2D grid world with terrain costs, static blocks, and moving obstacles."""
    ...
    def set_terrain_region(self, x1: int, y1: int, x2: int, y2: int, terrain: TerrainType):
        """Fill a rectangular region with the given terrain type (inclusive bounds)."""
    ...
    def is_valid_position(self, x: int, y: int) -> bool:
        """Return True if the coordinate lies within the grid bounds."""
        return 0 <= x < self.width and 0 <= y < self.height
    
    def is_passable(self, x: int, y: int, time: int = None) -> bool:
        """Return True if the cell is not blocked at the given time."""
