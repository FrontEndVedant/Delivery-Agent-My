...
class DeliveryAgent:
    """Autonomous delivery agent capable of planning and delivering packages.

    This class encapsulates the agent state, task queue, and the selected
    search-based planning strategy. It exposes small, purpose-led methods so the
    simulation loop stays simple and easy to reason about.
    """
    ...
    def set_planning_strategy(self, strategy: PlanningStrategy):
        """Set the planning strategy for the agent and instantiate its planner."""
    ...
    def _create_algorithm(self, strategy: PlanningStrategy) -> SearchAlgorithm:
        """Create a concrete search algorithm for the given strategy."""
    ...
    def add_delivery_task(self, task: DeliveryTask):
        """Add a delivery task and keep tasks ordered by priority."""
    ...
    def plan_next_action(self) -> bool:
        ...
        Returns True if a path was found and staged; False if no tasks remain or
        planning failed.
        """
    ...
    def _determine_next_goal(self) -> Optional[Position]:
        """Choose the next goal position based on current state and tasks."""
    ...
    def execute_next_action(self) -> bool:
        """Advance one step along the current path, replanning if needed."""
    ...
    def _is_path_still_valid(self) -> bool:
        """Return True if the remaining path is still traversable in time."""
        if not self.current_path or self.path_index >= len(self.current_path):
            return False
    ...
    def _move_to_position(self, position: Position):
        """Move the agent to the specified position and update bookkeeping."""
    ...
    def _check_task_completion(self):
        """Handle pickups and deliveries available at the current position."""
        for task in self.tasks:
    ...
    def run_simulation(self, max_steps: int = 1000) -> Dict[str, Any]:
        """Run the delivery loop for up to max_steps or until fuel is exhausted."""
    ...
    def reset(self):
        """Reset the agent to its initial state and clear all counters."""
        self.state = AgentState(
