...
@dataclass
class SearchNode:
    """Node within the search frontier/tree."""
    position: Position
...
class SearchResult:
    """Container for the outcome and metrics of a search run."""
...
class SearchAlgorithm:
    """Base class for search algorithms (BFS, UCS, A*, local search)."""
...
class BFS(SearchAlgorithm):
    """Breadth-first search over the time-extended grid."""
...
class UniformCostSearch(SearchAlgorithm):
    """Uniform-cost search minimizing cumulative movement cost."""
...
class AStar(SearchAlgorithm):
    """A* search guided by an admissible heuristic."""
    ...
    def manhattan_distance(self, pos1: Position, pos2: Position) -> float:
        """Admissible Manhattan heuristic for 4-connected movement."""
        return abs(pos1.x - pos2.x) + abs(pos1.y - pos2.y)
    
    def euclidean_distance(self, pos1: Position, pos2: Position) -> float:
        """Admissible Euclidean heuristic (often less informed here)."""
        return ((pos1.x - pos2.x) ** 2 + (pos1.y - pos2.y) ** 2) ** 0.5
    
    def diagonal_distance(self, pos1: Position, pos2: Position) -> float:
        """Admissible Chebyshev-like heuristic for 8-connected movement."""
...
class HillClimbingSearch(SearchAlgorithm):
    """Greedy hill-climbing with occasional random walks and restarts."""
    ...
    def _hill_climb(self, start: Position, goal: Position, 
        ...
        """Perform one hill-climb run, allowing brief random exploration."""
        current_pos = start
...
class SimulatedAnnealingSearch(SearchAlgorithm):
    """Simulated annealing to explore and gradually refine paths."""
    ...
    def _simulated_annealing_run(self, start: Position, goal: Position, 
        ...
        """Carry out one annealing pass from an initial candidate path."""
    ...
    def _generate_initial_path(self, start: Position, goal: Position, 
        ...
        """Seed an initial path using A*, falling back to a random walk."""
        # Try A* first
        try:
    ...
    def _generate_random_path(self, start: Position, goal: Position, 
                            start_time: int) -> List[Position]:
        """Generate a plausible random path constrained by passability."""
        path = [start]
        current_pos = start
        current_time = start_time
    ...
    def _generate_neighbor_path(self, path: List[Position], 
                              start_time: int) -> List[Position]:
        """Propose a nearby path by locally re-planning a segment."""
        if len(path) < 2:
            return path.copy()
    ...
    def _calculate_path_cost(self, path: List[Position]) -> float:
        """Compute the cumulative terrain cost of a path."""
        if len(path) < 2:
            return 0.0
    ...
    def _acceptance_probability(self, current_cost: float, new_cost: float, 
        ...
        """Probability of accepting a worse solution at a given temperature."""
